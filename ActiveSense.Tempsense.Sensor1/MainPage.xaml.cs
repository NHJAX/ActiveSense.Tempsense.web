...
namespace ActiveSense.Tempsense.Sensor1
{
    /// <summary>
    ...
    public sealed partial class MainPage : Page
    {
        ...
        private async void InitializeDeviceKeyAndName()
        {
            ...
            documento.LoadXml("~/Configs/SensorConfig.xml");
            XmlNodeList Devices = documento.GetElementsByTagName("Devices");
            XmlNodeList lista = ((XmlElement)Devices[0]).GetElementsByTagName("Device");
            foreach (XmlElement nodo in lista)
            {
                deviceKey = nodo.GetElementsByTagName("DeviceKey").ToString();
                deviceName = nodo.GetElementsByTagName("DeviceName").ToString();
                iotHubUri = nodo.GetElementsByTagName("IotHubUri").ToString();
                readingInterval = int.Parse(nodo.GetElementsByTagName("ReadingInterval").ToString());
                ambiente = nodo.GetElementsByTagName("Ambiente").ToString();
            }
        }
